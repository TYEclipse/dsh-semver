...
import { diffKind, incVersion, INC_RELEASES, normalizeVersion, parseVersion, relationLabel, satisfiesRange, sortVersionStrings, } from "./semver.js";
function renderParse(value) {
    const result = value;
    if (!result.valid)
        return `invalid version "${result.input}": ${result.reason ?? 'unknown reason'}`;
...
function renderCompare(value) {
    const result = value;
    if (!result.valid)
        return `cannot compare "${result.left}" vs "${result.right}": ${result.reason ?? 'unknown reason'}`;
    return result.verdict ?? '';
}
function renderSatisfies(value) {
    const result = value;
    if (!result.valid)
        return `cannot check "${result.version}" against "${result.range}": ${result.reason ?? 'unknown reason'}`;
    return result.explanation ?? '';
}
function renderInc(value) {
    const result = value;
    if (!result.valid)
        return `cannot increment "${result.version}" by ${result.release}: ${result.reason ?? 'unknown reason'}`;
    return `${result.version} +${result.release} → ${result.next ?? ''}${result.rule ? ` (${result.rule})` : ''}`;
}
function renderDiff(value) {
    const result = value;
    if (!result.valid)
        return `cannot diff "${result.left}" vs "${result.right}": ${result.reason ?? 'unknown reason'}`;
    return result.explanation ?? '';
}
...
export function buildSemverTools(config) {
    const semver_parse = defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [{ type: 'text', text: renderParse(value) }],
        },
        async execute(args) {
            ...
            if (value.prerelease !== null)
                out.prerelease = [...value.prerelease];
            if (value.build !== null)
    ...
    const semver_compare = defineTool({
        ...
        async execute(args) {
            ...
            if (!a.ok)
                return { valid: false, left: args.left, right: args.right, reason: `left ${a.reason}` };
            const b = parseVersion(args.right);
            if (!b.ok)
                return { valid: false, left: args.left, right: args.right, reason: `right ${b.reason}` };
    ...
    const semver_satisfies = defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [{ type: 'text', text: renderSatisfies(value) }],
        },
        async execute(args) {
            const parsed = parseVersion(args.version);
            if (!parsed.ok)
                return { valid: false, version: args.version, range: args.range, reason: parsed.reason };
            const result = satisfiesRange(parsed.value, args.range, args.includePrerelease ?? config.includePrerelease);
            if (!result.ok)
                return { valid: false, version: args.version, range: args.range, reason: result.reason };
    ...
    const semver_inc = defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [{ type: 'text', text: renderInc(value) }],
        },
        async execute(args) {
    ...
    const semver_diff = defineTool({
        ...
        async execute(args) {
            const a = parseVersion(args.left);
            if (!a.ok)
                return { valid: false, left: args.left, right: args.right, equal: false, reason: `left ${a.reason}` };
            const b = parseVersion(args.right);
            if (!b.ok)
                return { valid: false, left: args.left, right: args.right, equal: false, reason: `right ${b.reason}` };
    ...
    const semver_sort = defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [{ type: 'text', text: renderSort(value) }],
        },
        async execute(args) {
            const order = args.order ?? 'asc';
            const result = sortVersionStrings(args.versions, order === 'desc' ? 'desc' : 'asc');
            if (!result.ok)
                return { valid: false, count: 0, order, sorted: [] };
            const out = {
